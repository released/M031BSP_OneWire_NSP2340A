...
void NSP_idx_set6_10(uint8_t idx) // index 6 ~ 10
{
    ...
    }while(!OneWire_AskStatus());
    #else
    OneWire_WaitPlayEND();
    #endif
        
    printf("OneWire_WaitPlayEND (%d)\r\n",idx);
...
void NSP_process(void)
{
    ...
    if(FLAG_NSP_STOP)
    {
        printf("(loop)trigger stop playing\r\n");
        FLAG_NSP_STOP = 0;	
        N_STOP(); 
        printf("(loop)stop playing\r\n");
    }
